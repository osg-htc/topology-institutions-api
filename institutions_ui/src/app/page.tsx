...
export default function InstitutionList() {
  ...
  const [showOnlyWithUnitIds, setShowOnlyWithUnitIds] =
    useState<boolean>(false);
  const [loading, setLoading] = useState<boolean>(true);

  useEffect(() => {
    setLoading(true);
    refreshInstitutions().finally(() => {
      setLoading(false);
    });
  ...
  const displayData = showOnlyWithUnitIds
    ? filteredInstitutions.filter((institution) => institution.unitid !== null)
            ...
            {loading ? (
              ...
              displayData.map((institution) => (
                <TableRow key={institution.id}>
                  <TableCell>
                    <Link
                      href={`/update-institution?id=${extractShortId(institution.id)}`}
                    >
                  ...
                  <TableCell>
                    {institution.ror_id ? (
                      ...
                      >
                        {deleteTrailingSlash(institution.ror_id) || 'N/A'}
                      </a>
                    ) : (
                      'N/A'
                    )}
